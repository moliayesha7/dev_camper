...
const Bootcamps = () => {
  ...
  useEffect(() => {
    const url =
      'http://115.127.8.84:8080/api/v1/bootcamps?page=' +
      currentPage +
      '&limit=5';
    fetch(url, {
      method: 'GET',
  ...
  const onClickHandler = (eventID) => {
    window.location.href = `singleBootcamp/${eventID}`;
  };
  ...
  const renderBootcampData = (bootcamp, index) => {
    return (
      <div
        className='card-b'
        id={index}
        onClick={() => onClickHandler(bootcamp.id)}
      >
        <div className='card-horizontal'>
          ...
          <div className='card-body'>
            ...
            <p className='btn btn-dark'>Boston,MA</p>

            <p>
              {bootcamp.courses.map((course, index) => {
                return renderCourses(course, index);
              })}
            </p>
          </div>
        </div>
  ...
  const renderCourses = (course, index) => {
    return (
      <span className='card-text font-weight-bold' id={index}>
        {course.title},
      </span>
    );
  };
    ...
    <div className='row bootcamps'>
      <div className='col-4'>
        ...
        <div className='form-group'>
          ...
          <button className='btn-custom btn-block mt-2'>Find Bootcamps</button>
        </div>
      </div>

      <div className='col-8'>
        {bootcamps.map((bootcamp, index) => {
          return renderBootcampData(bootcamp, index);
        })}
